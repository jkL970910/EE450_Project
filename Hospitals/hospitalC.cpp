...
class Hospital {
    ...
    void findLocationScore(int location) {
        ...
        // on-screen message 5
        fprintf(stderr, "Hospital C has capacity = %d, occupation = %d, availability = %g\n", this->capacity, this->occupancy, a);
        float d;
        ...
        if (d == -1 || a == -1)  {
            // on-screen message 3
            if (d == -1) fprintf(stderr, "Hospital C does not have the location %d in map\n", location);
            hospitalScore[0] = -1;
            hospitalScore[1] = -2;
            return; 
        }
...
class SchedulerMain {
    ...
    void connectScheduler(Hospital& hospital) {
        ...
        while (1) {
            ...
            // on-screen message 2
            fprintf(stderr, "Hospital C has received input from client at location %d\n", location);
            ...
            if (hospitalScore[1] == -2) fprintf(stderr, "Hospital C has sent 'location not found' to the Scheduler\n");
            else if (hospitalScore[1] == -1) fprintf(stderr, "Hospital C has sent score = None and distance = None to the Scheduler\n");
            else fprintf(stderr, "Hospital C has sent score = %g and distance = %g to the Scheduler\n", hospitalScore[0], hospitalScore[1]);
...
int main(int argc, char* argv[]) {
    ...
    fprintf(stderr, "Hospital C is up and running using UDP on port %d\n", HOSPITALC_PORT_NUM);
